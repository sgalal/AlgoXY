...
这段程序运行良好。但是，如果我们把标准库提供的\texttt{TreeMap}换成普通的二叉搜索树，程序的性能就变差了。尤其是搜索诸如Zara、Zed、Zulu等姓名时特别明显。
...
如前面单词统计的例子所示，有很多集合（set）和map容器是使用红黑树来实现的。包括Java标准库。

由于只增加了一个颜色信息，我们可以复用二叉搜索树的节点定义。如下面的Java代码所示：
...
对于函数式环境，我们可以在代数数据类型（algebraic data type）的定义中增加颜色参数，如下面的Scala例子所示：
...
在支持模式匹配的语言中，例如Scala，插入算法可以实现为下面的程序：
...
\begin{itemize}
\item 使用Java来实现本节介绍的插入算法。由于没有Java中模式匹配，需要分别检查四种不同的情况。
\end{itemize}
...
为了使用双重黑色概念，我们需要修改一下红黑树的定义，如下面的Scala示例代码：
...
case object R extends Color
case object B extends Color
case object BB extends Color  //双重黑色
\end{lstlisting}
...
删除算法中，唯一还没有定义的函数是$fixBlack^2$。我们需要在这个函数中，通过树的旋转操作和重新染色，最终去掉“双重黑色”。这里有三种情况需要处理。每一种情况中，双重黑色的节点即可以是普通节点，也可以是双重黑色的空节点$\Phi$。为此，我们需要增加一个双重黑色空节点的定义。
...
函数\texttt{addBlack}将红色节点染成黑色，将黑色节点染成双重黑色。它同样可以对空节点重新染色。
...
综合公式（\ref{eq:db-case-1a}）、（\ref{eq:db-case-2}）和（\ref{eq:db-case-3a}），我们可以得到最终的Scala删除程序。
...
\begin{itemize}
\item 实现本节提到的“标记――重建”删除算法：也就是先将要删除的节点标记，但不进行真正的删除。当被标记的节点数目超过50\%的时候，用全部未标记的节点重建树。
...
当插入一个新节点时，我们将其染成红色，然后修复平衡并返回。上述算法可以转换为下面的Java例子程序。

\lstset{language=Java}
\begin{lstlisting}
public Node insert(Node t, int key) {
    Node root = t;
    Node x = new Node(key);
    Node parent = null;
    while (t != null) {
