...
如果双重黑色的节点是双重黑色空节点$\Phi$，经过这样的旋转操作后，可以将其回复为普通空节点。为此我们为式\ref{eq:db-case-1}增加双重黑色空节点的处理：
...
fixBlack^2(T) = \left \{
  ...
  {r@{\quad:\quad}l}
  (\mathcal{C}, (\mathcal{B}, mkBlk(A), x, B), y, (\mathcal{B}, C, z, D)) & p 1.1 \\
  (\mathcal{C}, (\mathcal{B}, \phi, x, B), y, (\mathcal{B}, C, z, D)) & p 1.1a \\
  (\mathcal{C}, (\mathcal{B}, A, x, B), y, (\mathcal{B}, C, z, mkBlk(D))) & p 1.2 \\
  (\mathcal{C}, (\mathcal{B}, A, x, B), y, (\mathcal{B}, C, z, \phi)) & p 1.2a \\
  \end{array}
\right .
\label{eq:db-case-1a}
\ee

其中$p 1.1a$和$p 1.2a$的模式定义如下：

\[
p 1.1a : \left \{ \begin{array}{l}
