...
Red-black tree is a type of self-balancing binary search tree\cite{wiki-rbt}.
...
Summarizing the above functions yields the following Scala
program.
...
In order to support this, we need add defintion for doubly black empty node.
...
The \texttt{addBlack} function turns a red nod to black, and turns a black node
to doubly black. It also applies to the empty node.
...
Let's examine the following three cases one by one.
...
together, we can write the final Scala program.
...
before return. Below is the example Java program.
...
Below example Java program implements the fixing algorithm after insertion.
...
We put the red-black tree delete algorithm in imperative settings in Appendix C,
