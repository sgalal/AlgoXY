...
和整数Trie一样，字符Trie的空间利用率很低。我们可以用同样的方法将字符Trie压缩成前缀树。

% ================================================================
%                 Definition of Alphabetic 前缀树 Tree
% ================================================================
\subsection{定义}

字符前缀树是一种特殊的前缀树，每个节点包含若干分支。所有的子节点拥有一个最长公共前缀串。Patrica中不存在只含有一个子分支的节点，否则最长公共前缀的长度就可以增加，因而和“最长”的性质相矛盾。

如果把图\ref{fig:trie-of-26}中Trie的所有仅含有一个子分支的节点压缩，转换成前缀树，可以得到一棵如图\ref{fig:patricia-tree}的前缀树前缀树。

\begin{figure}[htbp]
  \centering
  \includegraphics[scale=0.5]{img/patricia-tree.ps}
  \caption{一棵前缀树前缀树，含有key：a、an、another、bool、boy和zoo}
...
记前缀树为$T = (v, C)$，函数$insert(T, k, v')$将key$k$和数据$v'$插入到$T$中。
