...
concatenate the prefix in front of the key of every child sub-tree under $T_1$; If $k_1$ is prefix
of $k$, the algorithm recursively find all children sub-trees start with $k - k_1$;
else if $k$ is prefix of $k_1$, all children sub-trees under $T_1$
are valid result. Otherwise, the algorithm by-passes the first child sub-tree
...
Different with the lazy programming environment like Haskell, however,
Scala
