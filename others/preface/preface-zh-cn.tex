...
这一方法比之前的暴力解法快很多。在我的计算机上，相应的程序平均只用0.02秒，就可以在十万个整数中找到答案。

我们还可以继续优化。每次查找，我们都要申请长度为$n+1$的数组；查找结束后，这个数组又被释放掉了。反复的申请和释放会占用不少时间。我们可以预先准备好足够长的数组，然后每次查找都复用它。另外，我们可以使用二进制的位来保存标志，这样能节约不少空间。下面的Java语言例子程序实现了这两点小改进。
...
在我的计算机上，这段程序处理一百万个整数，平均用时仅仅0.023秒。
...
在有些函数式编程语言，例如Scala中，划分一个序列已经被作为库函数提供了\footnote{划分的详细实现可参考附录A}。下面的例子代码实现了分而治之的算法\footnote{相应的Haskell例子代码可参考附录B}。
...
使用命令式编程语言的读者可能会担心这种实现的性能。对于最小的可分配ID问题，递归的深度为$O(\lg n)$，于是调用栈的大小也是$O(\lg n)$。因此空间复杂度并不能被忽略。实际上，我们可以通过将递归转换为迭代来避免空间上的占用\footnote{由于我们的函数是尾递归形式，大多数函数式编程语言会自动转换优化尾递归函数。}，如下面的Java语言例子程序。
...
这一程序运行快速并且不需要额外的栈空间。但是和前面的Scala程序比起来，并不那么直观、简洁，需要仔细阅读。有时我们需要在简洁与性能之间进行平衡。

\section{正规数――数据结构的威力}

如果说最小可用ID问题还有一些应用价值，那么接下来这个问题就纯粹是为了“有趣”了。我们要寻找第1500个正规数。所谓正规数，就是只含有2、3或5这三个因子的自然数\footnote{又名哈明数，在数论中称为5-光滑数。}。前三个正规数按照定义分别是2、3和5。数字$60 = 2^23^15^1$是第25个正规数。数字$21 = 2^03^17^1$由于含有因子7，所以不是正规数。前10个正规数如下表：

2,3,4,5,6,8,9,10,12,15

如果我们认为$1=2^03^05^0$也是一个合法的正规数，则1就是第一个正规数。

\subsection{暴力解法}

这道题目看起来并不复杂，我们可以从1开始，逐一检查所有自然数，对于每个整数，我们用除法把所有的2、3和5的因子都去掉，如果结果是1，则找到了一个正规数，当遇到第$n=1500$个正规数时就找到答案了。
...
这一暴力解法对于较小的$n$没有问题。但是根据这个方法编写的C语言程序，在我的计算机上耗时40.39秒才找到了第1500个正规数(859963392)。当试图求第15000个正规数时，程序运行了10分钟也没能找到答案，我只好把它强行停止。
...
我们从1开始，分别乘以2或3或5来生成整数。这样问题就变成如何依次生成正规数。我们可以使用队列这种数据结构来解决这个问题。
...
\begin{figure}[htbp]
  ...
  \caption{使用队列依次生成正规数的前4个步骤} \label{fig:queues}
\end{figure}
...
在一些函数式编程语言，例如Scala中，可以使用流（Stream）来表示无穷序列，如下面的例子程序所示\footnote{Haskell语言默认支持惰性求值，相应的例子程序请参考附录B}。
...
通过求\texttt{RegNum.ns.take(1500).last}，我们可以得到第1500个正规数859963392。
...
我们不断从这三个队列中取出最小的，直到取出第$n$个元素。图\ref{fig:q235}给出了构造正规数的前4步。
...
\begin{figure}[htbp]
  ...
  \caption{使用三个队列$Q_2$、$Q_3$和$Q_5$来构造正规数的前4步}
  \label{fig:q235}
\end{figure}
...
算法循环$n$次，每次循环，它从三个队列中取出最小的一个元素，这一步需要常数时间。接着它根据取出元素所在的队列，产生一到三个新元素放入队列，这一步也是常数时间。因此整个算法是$O(n)$的。按照此算法实现的Java程序如下，它仅用了不到1毫秒就输出了第1500个正规数859963392。
...
下面的Scala程序实现了上面的定义\footnote{常见的Scala程序通常使用可变（mutable）的队列、序列来提高性能。这里给出的是“纯”函数式风格的例子}。
...
执行\texttt{take(1500).last}就可输出答案859963392。
...
我们首先介绍数据结构中的“Hello world”――二叉搜索树，接下来讲解如何解决二叉树的平衡问题。然后介绍更多有趣的树，包括基数树和B树。

第二部份是关于堆的。我们给出一个抽象堆的定义，然后介绍使用数组和各种二叉树实现的二叉堆（Binary Heap）。接着扩展到其他的堆包括二项式堆、斐波那契堆和Pairing堆。

数组和队列通常被认为是简单的数据结构，但我们将在第三部份看到，它们实现起来并不简单。
...
\begin{Exercise}
\begin{itemize}
\item 对于最小可用ID问题，也可以使用命令式的方法来获得线性时间、常数空间的解。下面给出了两种思路：
  \begin{itemize}
  \item 鸽笼排序。可以将第$i$个数字$x$移动到它应在的位置，也即第$x$个位置。如果$x > n$，我们可以忽略它。否则，我们将第$i$个位置上的数和第$x$位置上的数交换。由于我们移动到第$i$个位置上的树可能不等于$i$，我们需要重复这一过程直到数字等于$i$或者超过$n$。令$i$从1递增到$n$，依次处理所有的数字。由于每个数字做多被交换一次，这一过程需是线性时间的（你能证明这一点么？）。接下来，再次扫描一次数组，如果在第$i$个位置的元素不等于$i$，我们就找到了答案。否则，说明这个数组是“满”的，我们返回$n+1$作为最小可用的数字。
  \item 符号编码。我们想知道一个数字存在与否，这是一种二值信息。可以把这种信息用正负号来表示。如果数字$x$存在于序列中，我们将第$x$个位置上的数字变为负数。当把所有不超过$n$的数字都做好标记后，我们可以再次从左至右扫描一遍数组，找到第一个正数作为答案。
  \end{itemize}
使用命令式编程语言实现这两种方法。
\item 当Andrew Koening在Dr. Dobbs杂志上讨论哈明数问题时，Cassio Neri给出了一种“扁平化”的方法来实现惰性的无穷序列间的归并。这一方法可以描述如下：
\begin{algorithmic}[1]
\State $H \gets \{1, ...\}$
\State $n \gets 0$
