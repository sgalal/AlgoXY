...
support model the lazy evaluation with stream, The above infinity series function can be translate into the following program.
...
And these functional definition can be realized in Scala as the following.
...
Run \texttt{get(1500).last} will generate the correct answer 859963392.
...
//TODO:
1. Move the reference to the end of the book
2. Add excecises:
   1, pigeonhole sort based min-free number solution
   2, sgn encoding solution for min-free number puzzle;
   3, Imperative flattened merge method for Hamming number
