...
我们也可以不用基于指针或者引用的数据结构，而通过另一个索引数组来实现链表。对于任何数组元素$A[i]$，$Next[i]$保存了$A[i]$下一个元素的索引。也就是说$A[Next[i]]$是$A[i]$的下一个元素。我们还需要一个变量来记录链表的表头。即索引$i_0$是的$A[i_0]$是链表中的第一个元素。我么可以使用$Next[\perp] = i_0$来代表这一特殊变量。我们还需要使用一个变量记录链表的表头索引$i_0$，使得$A[i_0]$表示链表的第一个元素。我们定义$Next[\perp] = i_0$。
...
\begin{figure}[htbp]
  ...
  \begin{tikzpicture}[scale=0.8]
    ...
    \draw[thick, ->] (n6) edge [bend left] (a2)
                 ...
                 (hd) edge (n6);
  \end{tikzpicture}
  \caption{使用索引表表示的链表} \label{fig:indexed-link-list-isort}
\end{figure}
...
其中函数\textproc{Insert-Tree}($T, x$)和\textproc{To-List}($T$)的定义在上一章。
